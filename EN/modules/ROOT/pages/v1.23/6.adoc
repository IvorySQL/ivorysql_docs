...
```
wget https://github.com/IvorySQL/IvorySQL/releases/download/Ivory_REL_1_17/IvorySQL-1.23-3402884-20260709.x86_64.rpm
```
...
rpm -ivh IvorySQL-1.23-3402884-20260709.x86_64.rpm
```
...
ExecStart=/opt//bin/pg_ctl start -D ${PGDATA}
ExecStop=/opt//bin/pg_ctl stop -D ${PGDATA}
ExecReload=/opt//bin/pg_ctl reload -D ${PGDATA}
...
rm -fr /opt/                             --remove residual installation                                                             directory
```
...
```
wget https://github.com/IvorySQL/IvorySQL/releases/download/IvorySQL_1.23/IvorySQL-1.23-3402884-20260709.amd64.deb
```
...
```
sudo apt install ./IvorySQL-1.23-3402884-20260709.amd64.deb
```
...
umask 022
export LD_LIBRARY_PATH=/opt//lib:$LD_LIBRARY_PATH
export PATH=/opt//bin:/usr/local/ivorysql//bin:$PATH #depend on your install path
export PGDATA=/home/ivorysql/data
```
...
mv /usr/lib/systemd/system/ivorysql.service /tmp/       --the service file can be moved                                                             to/tmp or deleted
rm -fr /usr/local/ivorysql/                --remove residual installation                                                             directory
```
