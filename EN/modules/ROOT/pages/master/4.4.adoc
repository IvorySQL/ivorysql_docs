...
The pg_upgrade tool is a built-in cross-version upgrade utility in PostgreSQL that enables in-place database upgrades without requiring export and import operations. Since IvorySQL is derived from PostgreSQL, it can also use the pg_upgrade tool for major version upgrades. Below is a brief introduction on how to use pg_upgrade to upgrade IvorySQL to the latest 5.0 version on the CentOS8 platform.
...
```
Then install the new version of the IvorySQL 5.0 database:
```
dnf install -y ivorysql5-5.0
```
Initialize the new IvorySQL 5.0 data directory:
```
