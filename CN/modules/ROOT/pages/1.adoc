...
- 使 `ivorysql.enable_case_switch` 在 `SHOW ALL` 中可见，并可通过配置文件进行配置：PR [#1450](https://github.com/IvorySQL/IvorySQL/pull/1450)
- 修复 `DBMS_UTILITY` 回溯和调用栈输出中缺少包名及模式限定错误的问题：PR [#1471](https://github.com/IvorySQL/IvorySQL/pull/1471)
- 修复 `REGEXP_COUNT`、`REGEXP_SUBSTR`、`REGEXP_INSTR` 和 `REGEXP_REPLACE` 中换行符处理错误以及 `m`/`n` 选项行为异常的问题：PR [#1489](https://github.com/IvorySQL/IvorySQL/pull/1489)
