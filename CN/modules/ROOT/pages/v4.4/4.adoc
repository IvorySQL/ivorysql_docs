...
|                `relid` `oid`被分析的表的OID。                
| `phase` `text`当前处理阶段。参见 http://www.postgresql.org/docs/17/progress-reporting.html#ANALYZE-PHASES[表  33]。 
...
| `phase` `text`索引创建的当前处理阶段。 参见 http://www.postgresql.org/docs/17/progress-reporting.html#CREATE-INDEX-PHASES[表  35]。 
