...
IvorySQL社区提供的是中英文文档。英文文档保存在docs/中，中文文档保存在i18n/zh-CN中。您可以为任一文档存储库做出贡献，并提交PR请求合并至： https://github.com/IvorySQL/Ivory-www。
...
IvorySQL 文档存储库: git@github.com:IvorySQL/ivorysql_docs.git

==== 回答邮件列表问题

邮件列表位于 官网下: https://lists.ivorysql.org/postorius/lists/

==== 贡献文章

您可以在IvorySQL-WWW代码仓库自己提交至blog贡献，也可以联系我们IvorySQL小助理发送至邮箱 	ivorysql-www@ivorysql.org 。

=== 如何做出贡献

==== 编码指南

您获得反馈和看到代码合并到项目中的机会在很大程度上取决于更改的粒度。如果您的想法发生了更大的变化，我们强烈建议您在花大量时间编写代码之前，先加入 hackers@ivorysql.org邮件列表，并与我们分享您的建议。即使您的建议得到社区的验证，我们仍然建议您将实际工作作为一系列小型的、独立的提交来完成。这使得评审员的工作更加容易，并提高了反馈的及时性。
...
​    所有贡献给IvorySQL的新功能都应该有与其一起贡献的回归测试覆盖。如果您不确定如何测试或记录您的工作，请在hackers@ivorysql.org	邮件列表中提出问题，社区的开发人员将尽力帮助您。
...
IvorySQL社区提供双语文档。英文文档保存在docs/中，中文文档保存在i18n/zh-CN中。您可以为任何一方文档做出贡献，当然您也可以为两方同时做出贡献。
...
是由 `Antora` 进行搭建的，在运行 `Antora` 之前，记得修改对应 `playbook.yml` 文件

image::16.png[]
image::17.png[]

完成上述流程之后，请在终端运行命令 `antora generate --to-dir ../demo antora-playbook-CN.yml --stacktrace`，然后耐心等待，当成功运行结束后，你就可以到../demo 中查看自己生成的网页了。

检查之后，你就可以开始提交PR，和之前的流程相同，感谢您对社区的贡献^ _ ^，我们会在审核过后，考虑是否更新网站。

=== 网页预览

（1）在IvorySQL-www的仓库中提交的每一个PR, 被成功merge后，PR会话中将产生一个Deploy Preview链接，点击链接即可预览网页。

（2）如果是在文档中心的仓库（ivorysql_docs.git）提交的PR, github的workflows会自动在IvorySQL-www仓库中产生一个对应的PR，剩下的和上面（1）流程相同。
